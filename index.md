...
# Our Mission
Our Mission with Spriggan is to free creators of all types from corporate control and censorship from the platforms that we publish our work on. Our goal is simple, to give creators complete artistic freedom by making censorship impossible. Spriggan is created and support by creators, for creators. It is completely decentralized and open-source, allowing creators to publish and sell their work directly to their audience without any middlemen or payment processors.

Our first target is the PC gaming market, and the multi-billion dollar goliath that is Steam, owned by Valve.

# What is Spriggan

The Idea for Spriggan started as an idea to create a video game marketplace that was completely decentralized and permissionless using Peer-to-Peer technologies like Bittorrent and blockchain, but the idea has expanded since then. Spriggan is an attempt to break apart the vertical monopoly that is inherent to centralized digital publishing of not only games but all forms of digital media.

![diagram](/assets/images/diagram1.png)

The ultimate goal of the project is to eliminate all corporate control over digital creators and create an open source, censorship resistant ecosystem that allows creators to control and profit from their own work directly without relying on companies/platforms to monetize their work.

It may be hard to see how this is fundamentally different from any other platform. In fact, as an artist, the goal is for it to work as just another platform. The key difference is that Spriggan is designed to be completely modular, so that any number of organizations or companies can come along and make their own marketplaces, or library apps, or anything else that could be totally intercompatible with the rest of the ecosystem without needing anyone's permission.

That is not something that is possible with current platforms, they are all walled gardens with no underlying technology tying them together. Spriggan is trying to create that technology so that when you buy a piece of content, you actually own it, and not just the right to download it from that specific platform.
...
The choice to use a blockchain is a clear one. It will not be easy, but there is no way to create a truly free and open market without using a blockchain.
...
### Not Proof-of-Stake
Proof-of-Stake (PoS) is often touted as a solution to bitcoins energy usage, but PoS has huge security flaws that make PoS blockchains very centralized. The most important number when measuring a blockchains security is the Nakamoto Coefficient, which in simple terms, is the number of the largest entities that would have to collude to take control of the blockchain. Most blockchains (including bitcoin, and other top blockchains by market cap) have an estimated NC of < 10, while Chia is by far the highest at ~90 as of writing. It is by far the most decentralized blockchain in existance.


### A Good Team Behind it
Chia was created by Bram Cohen, the inventor of the Bittorrent protocol. Bitcoin was inspired by and named after Bittorrent. Bram invented peer to peer technologies and is one of the most brilliant minds in computing. There is not a person on earth more qualified to design a public blockchain.
...
Using our open source tools, creators can create and manage their own private DataLayer data store of products, which can be shared with trusted marketplaces. This private data includes torrent files and passwords to decrypt their content, which stay private between the creator, trusted marketplaces, and customers who have purchased the content.
